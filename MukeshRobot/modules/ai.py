...
async def get_api_response(model_id, api_params, api_url):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return data.get(
                "content", f"Kesalahan: Respons kosong diterima dari {model_id} API."
            )
        else:
...
async def upscale_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.message.reply_to_message and update.message.reply_to_message.photo:
            ...
            await update.message.reply_text("Harap Reply gambar untuk meningkatkannya.")

    except Exception as e:
        logger.error(f"Gagal meningkatkan gambar: {e}")
        await update.message.reply_text(
            "Gagal meningkatkan gambar. Silakan coba lagi."
        )
